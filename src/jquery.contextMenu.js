...
// These notes are for chrome
// Presence of originalEvent appears to indicate a right click
// Absence of originalEvent appears to indicate a left click that has bubbled

// The above check is sufficient to stop events on right client, however insufficient
// to stop events on left click.  If a menu is triggered by right click and a left
// click bubbles through it'll open the menu (because we've removed the default event
// disabling above to support menus on nested components)
// The following check attempts to ensure only right click events make it through, and
// this is working in combination with full menu disabling
			if (e.data.trigger == 'right' && !e.originalEvent) {
				return;
			}
